...
ufo <- ufo %>% 
  separate(date, 
           into = c('event.date', 'event.time'), 
           sep = ' ') # 20 have no date, 1235 have no time
...
# note: assuming 1900s but events dates on site (http://www.nuforc.org/webreports/ndxevent.html) indicate a very small handful prior to the 1900s
ufo$event.year <- ifelse(ufo$event.year > 2019, ufo$event.year - 100, ufo$event.year) 
...
# save cleaned file
write_csv(ufo, 'output/ufo_clean.csv')
